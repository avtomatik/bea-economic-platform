...
def parse_periods(df: pd.DataFrame, frequency: str) -> list[tuple[int, str]]:
    periods = []
...
def build_arrow_table(
    df: pd.DataFrame, metadata: dict, provenance: dict, frequency: str
) -> pa.Table:
...
def insert_failure(con, failure: dict):
    con.execute(
        """
...
def ingest_sheet(con, df: pd.DataFrame, provenance: dict):
    frequency = detect_frequency(df)
    if frequency is None:
        return
    metadata = parse_metadata(df)
    arrow_table = build_arrow_table(
        df=df, metadata=metadata, provenance=provenance, frequency=frequency
    )
...
def main():
    ...
    print("DUCKDB_PATH:", DUCKDB_PATH.resolve())
    con = duckdb.connect(str(DUCKDB_PATH))
    create_tables(con)
    for archive_path in sorted(DATA_DIR_SRC.iterdir()):
        print()
        print("=" * 80)
            ...
            for member in archive.filelist:
                ...
                for sheet_name in excel.sheet_names:
                    ...
                    try:
                        ...
                        ingest_sheet(con=con, df=df, provenance=provenance)
